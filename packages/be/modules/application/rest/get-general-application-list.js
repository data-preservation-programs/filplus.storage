...
const getGeneralApplicationList = async (username, token, perPage) => {
  try {
...
MC.app.get('/get-general-application-list', async (req, res) => {
  try {
    ...
    const generalApplicationList = await getGeneralApplicationList(user.githubUsername, user.githubToken, perPage)
