...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      if (this.$refs.grid) {
        ...
        this.gridWidth = svg.clientWidth || 1109
        this.warp = new Warp(svg)
        this.warp.interpolate(4)
        this.warp.transform(([x, y]) => [x, y, x, y])
        this.interval = setInterval(() => {
          this.renderWarpFrame(this.frame)
          this.frame++
        }, 3000)
      }
    })
  ...
  methods: {
    renderWarpFrame (frame) {
      const width = this.gridWidth
      const scalar = Math.cos(frame * Math.PI) * 0.5
      this.warp.transform(([x, y, ox, oy]) => [
        ox - (gaussianSmoothStep(ox, oy, width) * scalar),
        oy - (gaussianSmoothStep(ox, oy, width) * scalar),
        ox,
        oy
      ])
    }
