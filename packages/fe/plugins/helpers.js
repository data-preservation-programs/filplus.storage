...
const FormatDatacapSize = (ctx, size, args) => {
  ...
  if (action === 'human') {
    ...
    const options = store.getters['general/siteContent'].index.page_content.form.scaffold.datacap_size_unit.options
...
const ReactDatasizeUnitToRange = (ctx, size, args) => {
  const options = ctx.store.getters['general/siteContent'].index.page_content.form.scaffold.datacap_size_unit.options
